...
// const input = `toggle 0,0 through 0,2
// toggle 0,0 through 4,0`
// const input = `turn on 0,1 through 0,2
// turn on 0,0 through 4,0
...
const createField = () => {
  ...
  const matrix = new Array(matrixSize).fill(null).map(() => [...row]);
  return matrix;
}

const applyMatrix = (matrix, p) => {
  for (x = p.x1; x <= p.x2; x++) {
    for (y = p.y1; y <= p.y2; y++) {
      if (p.op == "toggle") matrix[x][y] = matrix[x][y] ? 0 : 1
      if (p.op == "turn on") matrix[x][y] = 1;
      if (p.op == "turn off") matrix[x][y] = 0;
    }
  }
  return matrix;
}
...
const readInput = s => {
  [_, op, x1, y1, x2, y2] = [...s.matchAll(/([\w ]*) (\d*),(\d*) through (\d*),(\d*)/gm)][0]
  return {op, x1, x2, y1, y2}
}

const currentMatrix = createField()
const matrix = input
  .split("\n")
  .reduce((c, p) => applyMatrix(c, readInput(p)), currentMatrix)
const result = matrix
  .reduce((c, row) => c + row.reduce((c, i) => c + i ,0), 0)
const result2 = matrix
  .reduce((c, row) => c + row.reduce((c, i) => c + (1 - i) ,0), 0)
