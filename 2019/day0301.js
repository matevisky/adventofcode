...
const feedMap = (path, id) => {
  let x = 0;
  let y = 0;
  dist = 0;
  path.split(',').forEach(direction => {
    const directionArray = [...direction];
    const arrow = directionArray.shift();
    const distance = parseInt(directionArray.join(""));
    for (let i = 0; i < distance; i++) {
      if (!map[[x, y]]) {
        map[[x, y]] = { ids: [id], dist }
      } else if (map[[x, y]].ids[0] != id && map[[x, y]].ids.length == 1) {
        map[[x, y]].dist += dist
        map[[x, y]].ids.push(id)
      }
      if (arrow == 'R') {
...
const findShortest = () => {
  ...
  Object.keys(map).forEach(p => {
    if (map[p].ids.length == 2) {
      if (map[p].dist > 0 && shortest > map[p].dist) {
        shortest = map[p].dist;
      }
    }
  })
