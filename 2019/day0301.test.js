const feedMap = (map, path, id, mapResults) => {
  pointer = {
    x: 0,
    y: 0,
    distance: 0,
  ...
  decodePath(path).forEach(direction => {
    for (let i = 0; i < direction.distance; i++) {
      markMap(map, pointer)
      checkValid(map, pointer, mapResults)
      movePointer(pointer, direction)
    }
  })
  return map;
}

const markMap = (map, pointer) => {
  if(!map[[pointer.x, pointer.y]]) {
    map[[pointer.x, pointer.y]] = []
  }
  if (map[[pointer.x, pointer.y]].some(i => i.id == pointer.id)) return;
  map[[pointer.x, pointer.y]].push({ ...pointer });
}

const checkValid = (map, pointer, mapResults) => {
  if (pointer.x == 0 && pointer.y == 0) return;
  if (map[[pointer.x, pointer.y]].length > 1) {
    mapResults.push(map[[pointer.x, pointer.y]]);
  }
}

const movePointer = (pointer, direction) => {
  pointer.distance++;

  if (direction.arrow == "R") pointer.x++;
  if (direction.arrow == "L") pointer.x--;
  if (direction.arrow == "U") pointer.y++;
  if (direction.arrow == "D") pointer.y--;

  return pointer;
}

const findShortest = (mapResults) => {
  result = mapResults.reduce((shortest, pointers) => {
    const distance = pointers.reduce((sum, pointer) => sum + pointer.distance, 0)
    if (shortest == null) return distance;
    return distance < shortest ? distance : shortest;
  }, null);
...
const decodePath = (path) => {
  ret = path.split(',').reduce((current, direction) => {
    const arrow = direction[0];
    const distance = parseInt(direction.substr(1));
    current.push({arrow, distance})
    return current;
  }, []);
  return ret;
}
...
test("day03 distance result", () => {
  ...
  map = feedMap(map, path1, 1, mapResults);
  map = feedMap(map, path2, 2, mapResults);

  expect(findShortest(mapResults)).toBe(35194);
})
...
test("day03 tests", () => {
  ...
  expect(findShortest(mapResults)).toBe(610);
})

test("decodePath", () => {
  let path = "R75,D30"
  expect(decodePath(path)).toMatchObject([{arrow: "R", distance: 75}, {arrow: "D", distance: 30}])
})

test("movePointer", () => {
  let pointer = { distance: 1, id: "path1", x: 0, y: 0 }
  let direction = { arrow: "U", }
  let resultPointer = {distance: 2, id: "path1", x: 0, y: 1}
  expect(movePointer(pointer, direction)).toMatchObject(resultPointer)
  direction = { arrow: "R", }
  resultPointer = {distance: 3, id: "path1", x: 1, y: 1}
  expect(movePointer(pointer, direction)).toMatchObject(resultPointer)
  direction = { arrow: "D", }
  resultPointer = {distance: 4, id: "path1", x: 1, y: 0}
  expect(movePointer(pointer, direction)).toMatchObject(resultPointer)
  direction = { arrow: "L", }
  resultPointer = {distance: 5, id: "path1", x: 0, y: 0}
  expect(movePointer(pointer, direction)).toMatchObject(resultPointer)
})
