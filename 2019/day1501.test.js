...
const maxX = 50;
const maxY = 50;
const pos = { x: startX, y: startY, vector: 0 }
let stepCount = 0;

const generatePuzzle = (maxX, maxY, pos) => {
  const puzzle = []
  for (let y = 0; y < maxY; y++) {
    const buffer = [];
    for (let x = 0; x < maxX; x++) {
      let toPush = legend.UNKNOWN;
      if (pos.x == x && pos.y == y) toPush = legend.START
      buffer.push(toPush)
    }
    puzzle.push(buffer);
  }
...
const printPuzzle = (puzzle, pos) => {
  let buffer = "";
  for (let y = 0; y < maxY; y++) {
    for (let x = 0; x < maxX; x++) {
      let toPush = puzzle[y][x]
      if (pos.x == x && pos.y == y) toPush = legend.PLAYER
      buffer += toPush
    }
    buffer += "\n";
  }
...
const lookAround = (pos, c, i) => {
  [1, 2, 3, 4].forEach(j => {
    pos.vector = j;
    movePuzzle(pos, c, i, true);
  })
}

const movePuzzle = (pos, c, i, isLook = false, isStep = false) => {
  c.input = [pos.vector]
  i.next();
  const [r] = c.output;
  c.output = [];
  const observe = {
    x: pos.x + vectorMatrix[pos.vector].x,
    y: pos.y + vectorMatrix[pos.vector].y,
  }
  if (r == 0) {
    puzzle[observe.y][observe.x] = legend.WALL
  } else {
    puzzle[observe.y][observe.x] = r == 1 ? legend.EMPTY : legend.GO
    puzzle[startY][startX] = legend.START
    pos.x = observe.x
    pos.y = observe.y
    if (isLook) {
      if (pos.vector == 1) pos.vector = 2
      else if (pos.vector == 2) pos.vector = 1
      else if (pos.vector == 3) pos.vector = 4
      else if (pos.vector == 4) pos.vector = 3
      movePuzzle(pos, c, i)
    } else {
      if (isStep) stepCount++;
    }
...
const puzzle = generatePuzzle(maxX, maxY, pos)

const readlineSync = require('readline-sync');

const getPos = (key, pos) => {
  if (key === 'w') pos.vector = 1
  if (key === 's') pos.vector = 2
  if (key === 'a') pos.vector = 3
  if (key === 'd') pos.vector = 4
}


// ['w', 'd', 'w', 'd', 's', 'd', 's', 'a', 's', 'd', 's', 'd', 's', 'a',
//   's', 'd', 's', 'a',
//   's', 'a', 'w', 'a',
//   'w', 'd', 'w', 'a',
//   'w', 'a', 'w', 'a', 'w', 'a', 'w', 'd',
...
//   'a', 's', 'd', 's', 'a'
// ].forEach(keyPair => {
//   const [key, count] = keyPair.split(",")
//   for (let j = 0; j < 20; j++) {
//     if (count == j) break;
//     getPos(key, pos)
//     movePuzzle(pos, c, i, false, true)
//     lookAround(pos, c, i)
//   }
// })
// stepCount = 0;
...
//   'w', 'a,4', 's', 'a,2', 'w', 'a', 'w,6',
//   'd', 's', 'd', 'w', 'a', 'w', 'd', 'w,2', 'a',
//   'w', 'a', 's', 'a', 'w', 'd', 'w', 'a', 'w', 'd', 'w', 'd', 'w,2', 'a',
//   'w,2', 'd', 'w', 'd', 's', 'd', 'w', 'a', 's', 'a', 'w', 'a', 's', 'd', 's', 'a', 'w',
//   'd', 'w', 'd', 'w', 'a', 's', 'a', 's', 'd', 's', 'a', 'w', 'a', 'w', 'a',
//   'w', 'a', 's', 'd', 's', 'a', 's', 'd', 's', 'a', 's', 'd', 's', 'w', 'd', 'w', 'a',
//   'w', 'd', 's', 's', 'd', 's', 'd', 's', 'a', 's', 'd', 's', 'a', 's', 'd',
//   's', 'a', 'w', 'a', 's', 'a', 's', 'a', 'w', 'a', 'w', 'd', 'w', 'a', 'w', 'd',
// 'w', 'a', 's', 'a', 'w', 'a', 'w,2', 'a', 's', 'a', 's', 'd',
...
console.log(stepCount, pos);
console.log(printPuzzle(puzzle, pos));
let currrentMemory = [...c.memory]
while (true) {
  ...
  movePuzzle(pos, c, i, false, true)
  let newMemory = [...c.memory]
  const diff = []
  for (let j = 0; j < 1000000; j++) {
    if (currrentMemory[j] != newMemory[j] && newMemory[j] < 30 && currrentMemory[j]< 30)
      diff.push({j, old: currrentMemory[j], new: newMemory[j]})
  }
  currrentMemory = newMemory
